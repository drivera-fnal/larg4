...
//              geant4.10.03.p03/source/processes/hadronic/models/cascade/cascade/src/G4CascadeColliderBase.cc

#include "MyG4CascadeColliderBase.hh"
#include "MyG4InteractionCase.hh"
...
//#include "Geant4/G4InteractionCase.hh"
...
MyG4CascadeColliderBase::MyG4CascadeColliderBase(const G4String& name, G4int verbose)
  : G4VCascadeCollider(name, verbose), balance(0) {
  if (G4CascadeParameters::checkConservation())
    balance = new G4CascadeCheckBalance(name);
}

MyG4CascadeColliderBase::~MyG4CascadeColliderBase() {
  delete balance;
}

void MyG4CascadeColliderBase::setVerboseLevel(G4int verbose) {
...
G4bool MyG4CascadeColliderBase::validateOutput(G4InuclParticle* bullet,
					     G4InuclParticle* target,
...
G4bool MyG4CascadeColliderBase::validateOutput(const G4Fragment& fragment,
					     G4CollisionOutput& output) {
  if (!balance) return true;		// Skip checks unless requested
