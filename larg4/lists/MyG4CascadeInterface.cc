...
// ********************************************************************
//
// $Id: MyG4CascadeInterface.cc 71719 2013-06-21 00:01:54Z mkelsey $
//
...
MyG4CascadeInterface::MyG4CascadeInterface(const G4String& name)
...
void MyG4CascadeInterface::DumpConfiguration(std::ostream& outFile) const {
  G4CascadeParameters::DumpConfiguration(outFile);
}

void MyG4CascadeInterface::clear() {
  bullet=0;
  target=0;
}
...
void MyG4CascadeInterface::Initialize() {
...
void MyG4CascadeInterface::useCascadeDeexcitation() {
  collider->useCascadeDeexcitation();
}

void MyG4CascadeInterface::usePreCompoundDeexcitation() {
...
void MyG4CascadeInterface::SetVerboseLevel(G4int verbose) {
...
G4HadFinalState* 
MyG4CascadeInterface::ApplyYourself(const G4HadProjectile& aTrack, 
				  G4Nucleus& theNucleus) {
  if (verboseLevel)
    G4cout << " >>> MyG4CascadeInterface::ApplyYourself" << G4endl;

  if (aTrack.GetKineticEnergy() < 0.) {
    G4cerr << " >>> MyG4CascadeInterface got negative-energy track: "
...
MyG4CascadeInterface::Propagate(G4KineticTrackVector* theSecondaries,
			      G4V3DNucleus* theNucleus) {
  if (verboseLevel) G4cout << " >>> MyG4CascadeInterface::Propagate" << G4endl;

  std::cout << "Inside MyG4CascadeInterface.cc" << std::endl;
#ifdef MYG4CASCADE_DEBUG_INTERFACE
  if (verboseLevel>=0) {
...
G4HadFinalState* 
MyG4CascadeInterface::NoInteraction(const G4HadProjectile& aTrack,
				  G4Nucleus& /*theNucleus*/) {
  if (verboseLevel) 
    G4cout << " >>> MyG4CascadeInterface::NoInteraction" << G4endl;
...
G4bool MyG4CascadeInterface::createBullet(const G4HadProjectile& aTrack) {
  ...
  if (0 == bulletType && 0 == bulletA*bulletZ) {
    if (verboseLevel) {
      G4cerr << " MyG4CascadeInterface: " << trkDef->GetParticleName()
...
G4bool MyG4CascadeInterface::createTarget(G4Nucleus& theNucleus) {
  return createTarget(theNucleus.GetA_asInt(), theNucleus.GetZ_asInt());
}

G4bool MyG4CascadeInterface::createTarget(G4V3DNucleus* theNucleus) {
...
G4DynamicParticle* 
MyG4CascadeInterface::makeDynamicParticle(const G4InuclNuclei& inuc) const {
  if (verboseLevel >= 2) {
...
void MyG4CascadeInterface::copyOutputToHadronicResult() {
  if (verboseLevel >= 1)
    G4cout << " >>> MyG4CascadeInterface::copyOutputToHadronicResult" << G4endl;
...
G4ReactionProductVector* MyG4CascadeInterface::copyOutputToReactionProducts() {
  if (verboseLevel >= 1)
    G4cout << " >>> MyG4CascadeInterface::copyOutputToReactionProducts" << G4endl;
...
void MyG4CascadeInterface::checkFinalResult() {
  balance->collide(bullet, target, *output);
...
G4bool MyG4CascadeInterface::coulombBarrierViolation() const {
...
G4bool MyG4CascadeInterface::retryInelasticProton() const {
...
G4bool MyG4CascadeInterface::retryInelasticNucleus() const {
...
#ifdef MYG4CASCADE_DEBUG_INTERFACE
  // Report on all retry conditions, in order of return logic
  G4cout << "Inside MyG4CascadeInterface.cc \n" 
