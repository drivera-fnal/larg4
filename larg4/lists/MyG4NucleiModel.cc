...
const G4double MyG4NucleiModel::small = 1.0e-9;
const G4double MyG4NucleiModel::large = 1000.;

// For convenience in computing densities
const G4double MyG4NucleiModel::piTimes4thirds = pi*4./3.;

// Zone boundaries as fraction of nuclear radius (from outside in)
const G4double MyG4NucleiModel::alfa3[3] = { 0.7, 0.3, 0.01 };
const G4double MyG4NucleiModel::alfa6[6] = { 0.9, 0.6, 0.4, 0.2, 0.1, 0.05 };

// Flat nuclear potentials for mesons and hyperons (GeV)
const G4double MyG4NucleiModel::pion_vp       = 0.007;
const G4double MyG4NucleiModel::pion_vp_small = 0.007;
const G4double MyG4NucleiModel::kaon_vp       = 0.015;
const G4double MyG4NucleiModel::hyperon_vp    = 0.030;
...
MyG4NucleiModel::MyG4NucleiModel()
...
MyG4NucleiModel::MyG4NucleiModel(G4int a, G4int z)
  : verboseLevel(0), nuclei_radius(0.), nuclei_volume(0.), number_of_zones(0),
    A(0), Z(0), theNucleus(0), neutronNumber(0), protonNumber(0),
    neutronNumberCurrent(0), protonNumberCurrent(0), current_nucl1(0),
    current_nucl2(0), gammaQDinterp(kebins),
...
MyG4NucleiModel::MyG4NucleiModel(G4InuclNuclei* nuclei)
  : verboseLevel(0), nuclei_radius(0.), nuclei_volume(0.), number_of_zones(0),
    A(0), Z(0), theNucleus(0), neutronNumber(0), protonNumber(0),
    neutronNumberCurrent(0), protonNumberCurrent(0), current_nucl1(0),
    current_nucl2(0), gammaQDinterp(kebins),
...
void MyG4NucleiModel::reset(G4int nHitNeutrons, G4int nHitProtons,
...
void MyG4NucleiModel::generateModel(G4InuclNuclei* nuclei) {
  generateModel(nuclei->getA(), nuclei->getZ());
}

void MyG4NucleiModel::generateModel(G4int a, G4int z) {
  if (verboseLevel) {
    G4cout << " >>> MyG4NucleiModel::generateModel A " << a << " Z " << z
	   << G4endl;
  }
...
void MyG4NucleiModel::fillBindingEnergies() {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::fillBindingEnergies" << G4endl;
...
void MyG4NucleiModel::fillZoneRadii(G4double nuclearRadius) {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::fillZoneRadii" << G4endl;
...
G4double MyG4NucleiModel::fillZoneVolumes(G4double nuclearRadius) {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::fillZoneVolumes" << G4endl;
...
void MyG4NucleiModel::fillPotentials(G4int type, G4double tot_vol) {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::fillZoneVolumes(" << type << ")" << G4endl;
...
G4double MyG4NucleiModel::zoneIntegralWoodsSaxon(G4double r1, G4double r2,
					       G4double nuclearRadius) const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::zoneIntegralWoodsSaxon" << G4endl;
  }
...
G4double MyG4NucleiModel::zoneIntegralGaussian(G4double r1, G4double r2,
					     G4double nucRad) const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::zoneIntegralGaussian" << G4endl;
  }
...
void MyG4NucleiModel::printModel() const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::printModel" << G4endl;
  }
...
G4double MyG4NucleiModel::getFermiKinetic(G4int ip, G4int izone) const {
  G4double ekin = 0.0;
...
G4LorentzVector
MyG4NucleiModel::generateNucleonMomentum(G4int type, G4int zone) const {
...
MyG4NucleiModel::generateNucleon(G4int type, G4int zone) const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::generateNucleon" << G4endl;
  }
...
G4InuclElementaryParticle
MyG4NucleiModel::generateQuasiDeuteron(G4int type1, G4int type2,
				    G4int zone) const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::generateQuasiDeuteron" << G4endl;
  }
...
void
MyG4NucleiModel::generateInteractionPartners(G4CascadParticle& cparticle) {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::generateInteractionPartners" << G4endl;
  }
...
generateParticleFate(G4CascadParticle& cparticle,
  ...
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::generateParticleFate" << G4endl;

  if (verboseLevel > 2) G4cout << " cparticle: " << cparticle << G4endl;

  // Create four-vector checking
#ifdef G4CASCADE_CHECK_ECONS
  G4CascadeCheckBalance balance(0.005, 0.01, "MyG4NucleiModel");	// Second arg is in GeV
...
G4bool MyG4NucleiModel::useQuasiDeuteron(G4int ptype, G4int qdtype) {
...
G4bool MyG4NucleiModel::passFermi(const std::vector<G4InuclElementaryParticle>& particles,
				G4int zone) {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::passFermi" << G4endl;
  }
...
G4bool MyG4NucleiModel::passTrailing(const G4ThreeVector& hit_position) {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::passTrailing " << hit_position << G4endl;
...
void MyG4NucleiModel::boundaryTransition(G4CascadParticle& cparticle) {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::boundaryTransition" << G4endl;
  }
...
void MyG4NucleiModel::choosePointAlongTraj(G4CascadParticle& cparticle) {
  if (verboseLevel > 1)
    G4cout << " >>> MyG4NucleiModel::choosePointAlongTraj" << G4endl;
...
G4bool MyG4NucleiModel::forceFirst(const G4CascadParticle& cparticle) const {
...
G4bool MyG4NucleiModel::isProjectile(const G4CascadParticle& cparticle) const {
  return (cparticle.getGeneration() == 0);	// Only initial state particles
}

G4bool MyG4NucleiModel::worthToPropagate(const G4CascadParticle& cparticle) const {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::worthToPropagate" << G4endl;
  }
...
G4double MyG4NucleiModel::getRatio(G4int ip) const {
  if (verboseLevel > 4) {
    G4cout << " >>> MyG4NucleiModel::getRatio " << ip << G4endl;
  }
...
G4double MyG4NucleiModel::getCurrentDensity(G4int ip, G4int izone) const {
...
G4CascadParticle
MyG4NucleiModel::initializeCascad(G4InuclElementaryParticle* particle) {
  if (verboseLevel > 1) {
    G4cout << " >>> MyG4NucleiModel::initializeCascad(particle)" << G4endl;
  }
...
void MyG4NucleiModel::initializeCascad(G4InuclNuclei* bullet,
				     G4InuclNuclei* target,
				     modelLists& output) {
  if (verboseLevel) {
    G4cout << " >>> MyG4NucleiModel::initializeCascad(bullet,target,output)"
	   << G4endl;
  }
...
G4double
MyG4NucleiModel::inverseMeanFreePath(const G4CascadParticle& cparticle,
...
G4double
MyG4NucleiModel::generateInteractionLength(const G4CascadParticle& cparticle,
...
G4double MyG4NucleiModel::absorptionCrossSection(G4double ke, G4int type) const {
...
G4double MyG4NucleiModel::totalCrossSection(G4double ke, G4int rtype) const
{
