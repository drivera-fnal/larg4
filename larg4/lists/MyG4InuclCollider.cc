...
//    geant4.10.03.p03/source/processes/hadronic/models/cascade/cascade/src/G4InuclCollider.cc
...
#include "Geant4/G4PreCompoundDeexcitation.hh"
...
void MyG4InuclCollider::useCascadeDeexcitation() {
  delete theDeexcitation;
  theDeexcitation = new G4CascadeDeexcitation;
  theDeexcitation->setVerboseLevel(verboseLevel);
}

void MyG4InuclCollider::usePreCompoundDeexcitation() {
  delete theDeexcitation;
  theDeexcitation = new G4PreCompoundDeexcitation;
  theDeexcitation->setVerboseLevel(verboseLevel);
}
...
void MyG4InuclCollider::collide(G4InuclParticle* bullet, G4InuclParticle* target,
			      G4CollisionOutput& globalOutput) {
  if (verboseLevel) G4cout << " >>> MyG4InuclCollider::collide" << G4endl;
...
void MyG4InuclCollider::rescatter(G4InuclParticle* bullet,
				G4KineticTrackVector* theSecondaries,
				G4V3DNucleus* theNucleus,
				G4CollisionOutput& globalOutput) {
  if (verboseLevel) G4cout << " >>> MyG4InuclCollider::rescatter" << G4endl;
...
void MyG4InuclCollider::deexcite(const G4Fragment& fragment,
			       G4CollisionOutput& globalOutput) {
  if (fragment.GetA_asInt() <= 1) return;	// Nothing real to be de-excited

  if (verboseLevel) G4cout << " >>> MyG4InuclCollider::deexcite" << G4endl;
...
G4bool MyG4InuclCollider::photonuclearOkay(G4CollisionOutput& checkOutput) const {
  ...
  if (verboseLevel>1)
    G4cout << " >>> MyG4InuclCollider::photonuclearOkay" << G4endl;
